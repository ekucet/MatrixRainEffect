    ...
    var body: some View {
        ...
        VStack {
            ForEach(0..<constant.count, id: \.self) { index in
                let char = Array(constant)[getRandomIndex(index: index)]
                Text(String(char))
        ...
        .onAppear {
            withAnimation(.linear(duration: 6).delay(.random(in: 0...2)).repeatForever(autoreverses: false)) {
                startAnimation = true
            }
        }
...
let constant = "0123456789abcdefghijklmnoprstuvwxyzabcquepaje123jdj09"
